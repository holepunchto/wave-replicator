...
| `ofdm`        | data 2.1 - 10.7 kHz as OFDM, control 18.8 - 21.7 kHz       | control band, inaudible        | 13 - 24 s       |
| `ofdm-silent` | one band, 18.8 - 21.7 kHz, as OFDM                         | the same band                  | 29 - 92 s       |

Measured on a MacBook, speaker to its own mic, writer and reader as separate processes, 1 KB as 4 blocks of 256 B, with no resends, over several runs for the `ofdm` modes, where one lost frame and its retry make the spread. Their modem is 4 to 5 times ggwave's raw rate (DQPSK 6 to 9 times): the rest goes on proofs, parity and the protocol's timers, which were sized for ggwave. `fast`, `silent` and `morse` share the inaudible band, so they hear each other's broadcasts. The `ofdm` modes use the same bands with a different modulation, so they only talk to each other.
