...
const packet = {
  ...
  decode(state) {
    ...
    return { id: c.fixed(4).decode(state), payload: c.buffer.decode(state) }
  }
}
...
module.exports = class Broadcast extends EventEmitter {
  constructor(air, opts = {}) {
    ...
    this.modem =
      opts.modem ?? bands.modem('OFDM_INAUDIBLE', { sampleRate: air.sampleRate, volume: opts.volume })
  ...
  send(payload) {
    ...
    if (bytes.byteLength > this.modem.maxPayload) {
      throw new Error('Broadcast is ' + bytes.byteLength + ' bytes, at most ' + this.modem.maxPayload)
    }
  ...
  _onaudio(samples) {
    for (const bytes of this._receiver.push(samples)) this._onpacket(bytes)
