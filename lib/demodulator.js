...
module.exports = class Demodulator extends Transform {
  ...
  _open(cb) {
    for (let channel = 0; channel < this.protocols.length; channel++) {
      ...
      if (ofdm.is(protocol)) {
        // a header says how long its packet stays on air, so the band is busy from then, not
        // only once the whole packet has been heard
        const onheader = (seconds) => this.push({ channel, busy: seconds })
        const receiver = new Receiver(ofdm.modem(protocol, this.opts), { onheader })
        this._receivers.push({ channel, receiver })
      } else {
