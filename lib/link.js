...
// packet: [message id, 2 bytes][index][k][shard]. A message that fits one packet goes as it is,
// a bigger one as k shards of the length prefixed message plus parity, any k of them rebuild it
const HEADER = 4
...
module.exports = class Link extends EventEmitter {
  ...
  _onpacket(packet) {
    if (packet.byteLength <= HEADER || packet[3] === 0) return
    this.stats.received++

    const id = (packet[0] << 8) | packet[1]
    const index = packet[2]
    const k = packet[3]
