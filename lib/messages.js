...
// asks the sender of a partly received message for more parity frames, id is unused
const repair = {
  preencode(state, m) {
    c.uint.preencode(state, m.message)
    c.uint.preencode(state, m.k)
    c.uint.preencode(state, m.need)
  },
  encode(state, m) {
    c.uint.encode(state, m.message)
    c.uint.encode(state, m.k)
    c.uint.encode(state, m.need)
  },
  decode(state) {
    return {
      message: c.uint.decode(state),
      k: c.uint.decode(state),
      need: c.uint.decode(state)
    }
  }
}

// an application message for everyone in earshot, id is unused
const broadcast = c.buffer

const bodies = [announce, want, data, repair, broadcast]
...
module.exports = { ANNOUNCE, WANT, DATA, REPAIR, BROADCAST, message }
