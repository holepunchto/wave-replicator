...
module.exports = class MorseBroadcast extends EventEmitter {
  constructor(air, opts = {}) {
    ...
    this.demodulator.on('burst-start', () => this._onburst(true))
    this.demodulator.on('burst-end', () => this._onburst(false))
  ...
  async _send(text) {
    ...
    this.emit('send-start', { text, seconds })
    await this.air.play(this._input, samples)
    this.emit('send-end', { text, seconds })
  }

  // a burst of sound at our frequency, from its start until a word gap of quiet. It is read once it
  // ends, and a message follows if it was Morse
  _onburst(start) {
    if (start && !this._hearing && !this._echo()) {
      this._hearing = true
      this.emit('receive-start')
    } else if (!start && this._hearing) {
      this._hearing = false
  ...
  _ondecoded(text) {
    if (!this._echo()) this.emit('message', text)
  }

  _echo() {
    const now = this.air.now
    this._sending = this._sending.filter((w) => now <= w.until + 60)
    return this._sending.some((w) => now >= w.from && now <= w.until)
  }
}
