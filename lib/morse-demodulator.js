...
// a tone is this many times louder than the band either side of it, noise and other modems
// spill over all of them alike. A burst is only reported once it held a tone this many units
const SIDE_HZ = 400
const TONAL = 3
const TONAL_UNITS = 2
...
module.exports = class MorseDemodulator extends Transform {
  constructor(morse) {
    ...
    this._k = coefficient(morse.frequency, morse.sampleRate)
    this._sides = [
      coefficient(morse.frequency - SIDE_HZ, morse.sampleRate),
      coefficient(morse.frequency + SIDE_HZ, morse.sampleRate)
    ]
  ...
  // a burst of sound is on the band and not over yet
  get busy() {
  ...
  _transform(pcm, cb) {
    ...
    for (; at + this.block <= samples.length; at += this.block) {
      const m = this._magnitude(samples, at, this._k)
      const side = Math.max(...this._sides.map((k) => this._magnitude(samples, at, k)))
      this._level(m, m > TONAL * side)
    }
  ...
  _level(m, tonal) {
    ...
    this._burst.push(m)
    if (tonal && m > floor * ACTIVE) this._tonal++
    if (!this._started && this._tonal >= TONAL_UNITS * this.unitBlocks) {
      this._started = true
    ...
    if (this._burst.length > MAX_SECONDS / BLOCK) {
      this._burst = null
      this._end()
    }
  }
  ...
  _end() {
    if (this._started) this.emit('burst-end')
    this._started = false
  }
...
function coefficient(frequency, sampleRate) {
  return 2 * Math.cos((2 * Math.PI * frequency) / sampleRate)
}
