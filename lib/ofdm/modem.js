...
module.exports = class Ofdm {
  constructor(opts = {}) {
    ...
    this._filter = bandpass(this.low - 400, this.high + 400, this.sampleRate, 255)
  }
  ...
  decode(samples) {
    const filtered = convolve(samples, this._filter)
    const packets = []
    const L = this.size / 2

    let from = 0
    while (true) {
      const start = this._find(filtered, from)
      if (start === -1) break

      const packet = this._read(samples, start)
      if (packet === null) {
      ...
      from = start + packet.symbols * this.symbolLength - L
    }
  ...
  _find(signal, from) {
    ...
    if (from + 2 * L + this.symbolLength * 3 >= n) return -1
    ...
    const threshold = 0.5
    for (let d = from; d + 2 * L < n - this.symbolLength * 3; d++) {
      ...
      if (metric > threshold) {
        ...
        return this._align(signal, best)
      }
  ...
  _read(samples, start) {
    const at = (s) => this._spectrum(samples, start - this.backoff + s * this.symbolLength)
    if (start - this.backoff + 3 * this.symbolLength > samples.length) return null

    let previous = at(1)
    const current = at(2)
    const headerSoft = this._soft(previous, current)
    previous = current
