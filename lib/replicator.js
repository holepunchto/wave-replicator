...
module.exports = class Replicator {
  ...
  _want(want) {
    ...
    this.link.schedule(key, () => ({ type: WANT, id: this.id, body: want }))
    this._track(key, want.upgrade, want.upgrade ? -1 : want.start, want.end, this._deadline(want))
  }

  // blocks a holder is pushing, count as asked for until they should have arrived
  _expect(start, end) {
    if (end <= start) return
    this._track(this._keyOf('push', start), false, start, end, this._deadline({ start, end }))
  }
  ...
  _track(key, upgrade, start, end, seconds) {
    ...
    this._wants.set(key, { upgrade, start, end, deadline })
  }
  ...
  // how long an answer takes at most: someone's random delay, then every packet of it on air
  _deadline(want) {
    const blocks = want.upgrade ? 1 + this.wave.batch : Math.max(1, want.end - want.start)
    return 4 * this.link.detect + blocks * PACKETS_PER_BLOCK * this.link.slot + this.link.detect
  }
